...
for i, row in xlsx.iterrows():
    if len(beforerow) > 0 and len(secondbeforerow) > 0:
        tempa = []
        for j in range(1, 8):
...
if not os.path.exists(modelfilename):
    ...
    model = tf.keras.Sequential([
      tf.keras.layers.Dense(10, activation=tf.nn.relu, input_shape=(14,)),  # 입력의 형태가 필요합니다.
