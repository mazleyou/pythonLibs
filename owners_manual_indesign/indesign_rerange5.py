...
newFile = r'C:\Users\daredevil\PycharmProjects\pythonLibs\owners_manual_indesign\NE1_new.indd'
...
# 원본 인디자인 파일 열기
originFile = r'C:\Users\daredevil\PycharmProjects\pythonLibs\owners_manual_indesign\NE1.indd'
originDoc = app.Open(originFile, ShowingWindow=False)
docBaseName = originDoc.Name

all_item_list = []
item_list = []
for myPage in originDoc.Pages:
    for myFrame in myPage.TextFrames:
        if myFrame.Contents != "":
            item_list.append('t')
            item_list.extend(list(map(int, myFrame.GeometricBounds)))
            item_list.append(myFrame.Contents)
            d1 = myFrame.Characters[1]
            ddd = myFrame.Paragraphs[1].AppliedFont.FullName
