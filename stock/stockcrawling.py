...
stock_xlsx = pd.read_excel('stock.xls', dtype = {'종목코드': str, '기업명': str, '액면가(원)': str, '총액(원)': str})
    ...
    try:
        ...
        for page in range(1, 43):
            ...
            for i, row in temp.iterrows():
                curs.execute(insert_sql, (dt, code, row["체결시각"], row['체결가'], row['전일비'], row['매도'], row['매수'], row['거래량'], row['변동량']))
...
stock_df = stock_xlsx.loc[:, ['종목코드', '기업명', '액면가(원)', '총액(원)']]
...
for i, row in stock_df.iterrows():
    ...
    curs.execute(sel_history_sql, (time, row['종목코드']))
    res = curs.fetchall()

    if len(res) == 0:
        if priceCheck(row['액면가(원)']) > 0:
            curs.execute(insert_history_sql, (time, row['종목코드'], 'START'))
            getStock(row['종목코드'], time)
            curs.execute(update_history_sql, ('END', row['종목코드'], time))
    else:
        if res[0][3] == "START":
            curs.execute(del_sise_sql, (time, row['종목코드']))
